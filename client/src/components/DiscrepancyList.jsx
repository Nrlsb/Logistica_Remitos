    ...
    const fetchRemitos = async () => {
        try {
            ...
            const discrepancyRemitos = response.data.filter(remito => {
                ...
                const hasClientReported = remito.discrepancies?.client_reported?.length > 0;
                return hasMissing || hasExtra || hasClientReported;
            });
                            ...
                            <div key={remito.id} className="relative group">
                                ...
                                <div className="flex flex-col md:flex-row md:items-end justify-between gap-4 mb-6">
                                    <div className="flex-1">
                                        ...
                                        <div className="flex flex-wrap gap-x-6 gap-y-2 mt-3 p-3 bg-gray-50/50 rounded-xl border border-gray-100/50">
                                            <div className="flex items-center gap-2">
                                                <span className="text-[10px] font-bold text-gray-400 uppercase tracking-tight">Preparó</span>
                                                <div className="flex items-center gap-1.5 py-1 px-2 bg-white rounded-lg border border-gray-100 shadow-sm">
                                                    <div className="w-5 h-5 rounded-md bg-brand-blue/10 flex items-center justify-center text-[10px] text-brand-blue font-black border border-brand-blue/20">
                                                        {(remito.prepared_by || 'U')[0].toUpperCase()}
                                                    </div>
                                                    <span className="text-xs font-black text-gray-700">{remito.prepared_by || '-'}</span>
                                                </div>
                                            </div>
                                            <div className="flex items-center gap-2">
                                                <span className="text-[10px] font-bold text-gray-400 uppercase tracking-tight">Escaneó</span>
                                                <div className="flex items-center gap-1.5 py-1 px-2 bg-white rounded-lg border border-gray-100 shadow-sm">
                                                    <div className="w-5 h-5 rounded-md bg-green-500/10 flex items-center justify-center text-[10px] text-green-600 font-black border border-green-500/20">
                                                        {(remito.created_by || 'S')[0].toUpperCase()}
                                                    </div>
                                                    <span className="text-xs font-black text-gray-700">{remito.created_by || 'Sistema'}</span>
                                                </div>
                                            </div>
                                            <div className="flex items-center gap-2">
                                                <span className="text-[10px] font-bold text-gray-400 uppercase tracking-tight">Achicó</span>
                                                <div className="flex items-center gap-1.5 py-1 px-2 bg-white rounded-lg border border-gray-100 shadow-sm">
                                                    <div className="w-5 h-5 rounded-md bg-purple-500/10 flex items-center justify-center text-[10px] text-purple-600 font-black border border-purple-500/20">
                                                        {(remito.packages_added_by || (remito.status === 'finalized' ? 'N' : 'P'))[0].toUpperCase()}
                                                    </div>
                                                    <span className="text-xs font-black text-gray-700">{remito.packages_added_by || (remito.status === 'finalized' ? '-' : 'Pendiente')}</span>
                                                </div>
                                            </div>
                                        </div>
                                    </div>
                                    <button
                                        ...
                                        className="inline-flex items-center gap-2 px-4 py-2 bg-brand-blue text-white font-bold text-sm rounded-xl shadow-sm hover:bg-brand-dark transition-colors active:scale-95 h-fit whitespace-nowrap"
                                    >
